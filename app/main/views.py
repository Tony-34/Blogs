...
@main.route('/')
def index():
    qoute =blog.query.all()
    job =blog.query.filter_by(category = 'Job').all() 
    event = Blog.query.filter_by(category = 'Events').all()
    advertisement = blog.query.filter_by(category = 'Advertisement').all()
    return render_template('index.html', job = job,event = event, blogs = blogs,advertisement= advertisement)
